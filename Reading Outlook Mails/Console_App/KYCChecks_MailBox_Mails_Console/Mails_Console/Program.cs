                    ...
                    if (mail != null)
                    {
                        try
                        {
                            ...
                            using (SqlConnection conn = new SqlConnection(connectionstringtxt))
                            {
                                ...
                                cmd.Connection = conn;
                                //cmd.CommandType = CommandType.Text;
                                cmd.CommandText = "INSERT INTO dbo.tbl_outlook_mails_daily_dotnet_kycchecks_mumbai_mailbox (Subject,ReceivedDateTime,Sender,Categories,CC,Importance,EntryID,UploadDateTime,IsUnread,[To]) VALUES (@Subject,@ReceivedDateTime,@Sender,@Categories,@CC,@Importance,@EntryID,@UploadDateTime,@IsUnread,@TO)";
